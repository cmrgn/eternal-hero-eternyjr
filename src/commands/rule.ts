...
export const RULES = {
  ...
  'Rule 1.3: No arguing': 'No unnecessary arguments or fights — handle disagreements respectfully.',
  'Rule 1.4: No doxxing':
  ...
  'Rule 3.2: No off topic': 'No irrelevant content (use the off-topic channels).',
  'Rule 3.3: No swearing': 'No swearing or attempting to circumvent the profanity filter.',
  'Rule 3.4: No cross-posting': 'No posting the same message in multiple channels.',
  'Rule 3.5: No NSFW/spam': 'No NSFW or spam content.',
  'Rule 3.6: No links': 'No links to other Discord servers without permission from the mods.',
  ...
  'Rule 4.2: No harrassing mods': 'No harassing, excessively pinging or taunting the moderators.',
}
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  return interaction.reply(`${user ? `${userMention(user.id)} ` : ''}${bold(number)}: ${message}`)
}
